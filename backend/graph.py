...
def clause_batch_node(state: ContractState):
    ...
    end = min(start + BATCH_SIZE, len(state["clauses"]))
    ...
    return {"analyzed_clauses": results}

def index_increment_node(state: ContractState):
    return {"current_index": state["current_index"] + BATCH_SIZE}
...
builder.add_conditional_edges(START, should_continue, ["clause_batch_node", END])
builder.add_edge("clause_batch_node", "increment_node")
builder.add_conditional_edges("increment_node", should_continue, ["clause_batch_node", END])

graph = builder.compile()

def process_contract_intelligence(clauses: List[dict]) -> dict:
    """Main entrypoint to run the intelligence graph over a list of clauses in batches."""
    if not clauses:
